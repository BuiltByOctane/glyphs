...
import electron, { clipboard, ipcMain as ipcMain$1, nativeImage, app as app$1, globalShortcut, Tray, Menu, BrowserWindow } from "electron";
...
function returnResults(it) {
  ...
  if (schemaEnv.$async) {
    gen.if((0, codegen_1$Q._)`${names_1$9.default.errors} === 0`, () => gen.return(names_1$9.default.data), () => gen.throw((0, codegen_1$Q._)`new ${ValidationError}(${names_1$9.default.vErrors})`));
  } else {
...
function getData($data, { dataLevel, dataNames, dataPathArr }) {
  ...
  for (const segment of segments) {
    if (segment) {
      data = (0, codegen_1$Q._)`${data}${(0, codegen_1$Q.getProperty)((0, util_1$O.unescapeJsonPointer)(segment))}`;
  ...
  function addEvaluatedFrom(source) {
    ...
    if (it.props !== true) {
      if (schEvaluated && !schEvaluated.dynamicProps) {
        ...
        it.props = util_1$M.mergeEvaluated.props(gen, props, it.props, codegen_1$O.Name);
      }
    }
    if (it.items !== true) {
      if (schEvaluated && !schEvaluated.dynamicItems) {
        ...
        it.items = util_1$M.mergeEvaluated.items(gen, items2, it.items, codegen_1$O.Name);
      }
    }
  }
}
...
const def$_ = {
  ...
  code(cxt) {
    ...
    const len = it.opts.unicode === false ? (0, codegen_1$L._)`${data}.length` : (0, codegen_1$L._)`${(0, util_1$L.useFunc)(cxt.gen, ucs2length_1$1.default)}(${data})`;
...
const def$Z = {
  ...
  code(cxt) {
    ...
    if ($data) {
      ...
      const regExpCode = regExp.code === "new RegExp" ? (0, codegen_1$K._)`new RegExp` : (0, util_1$K.useFunc)(gen, regExp);
      const valid2 = gen.let("valid");
      gen.try(() => gen.assign(valid2, (0, codegen_1$K._)`${regExpCode}(${schemaCode}, ${u}).test(${data})`), () => gen.assign(valid2, false));
      cxt.fail$data((0, codegen_1$K._)`!${valid2}`);
    } else {
...
const def$U = {
  ...
  code(cxt) {
    ...
    if ($data || schema && typeof schema == "object") {
      cxt.fail$data((0, codegen_1$F._)`!${(0, util_1$H.useFunc)(gen, equal_1$4.default)}(${data}, ${schemaCode})`);
    } else {
...
function startClipboardWatcher(mainWindow) {
  if (watcherInterval) clearInterval(watcherInterval);
  watcherInterval = setInterval(() => {
    try {
      const currentText = clipboard.readText();
      const currentImage = clipboard.readImage();
      const currentImageURL = currentImage.isEmpty() ? "" : currentImage.toDataURL();
      let isChanged = false;
      let newItem = null;
      ...
      if (isChanged && newItem) {
        ...
        if (newItem.type === "text") {
          history = history.filter((item) => item.content !== (newItem == null ? void 0 : newItem.content));
        }
...
function setupIpcHandlers(mainWindow) {
  ipcMain$1.handle("get-history", () => {
    return store.get("history") || [];
  });
  ipcMain$1.handle("toggle-pin", (_, id2) => {
    const history = store.get("history") || [];
    const index = history.findIndex((i) => i.id === id2);
    if (index !== -1) {
  ...
  ipcMain$1.handle("delete-item", (_, id2) => {
    let history = store.get("history") || [];
    history = history.filter((i) => i.id !== id2);
    store.set("history", history);
    return history;
  });
  ipcMain$1.handle("clear-history", () => {
    let history = store.get("history") || [];
    history = history.filter((i) => i.isPinned);
    store.set("history", history);
    return history;
  });
  ipcMain$1.handle("paste-item", (_, id2, asPlainText = false) => {
    const history = store.get("history") || [];
    const item = history.find((i) => i.id === id2);
    if (item) {
      if (item.type === "text") {
  ...
  ipcMain$1.handle("get-groups", () => {
    return store.get("groups") || [];
  });
  ipcMain$1.handle("add-group", (_, group) => {
    const groups = store.get("groups") || [];
    groups.push(group);
    store.set("groups", groups);
    return groups;
  });
  ipcMain$1.handle("update-group", (_, updatedGroup) => {
    const groups = store.get("groups") || [];
  ...
  ipcMain$1.handle("delete-group", (_, id2) => {
    let groups = store.get("groups") || [];
    groups = groups.filter((g) => g.id !== id2);
    store.set("groups", groups);
    const history = store.get("history") || [];
  ...
  ipcMain$1.handle("set-item-group", (_, itemId, groupId) => {
    const history = store.get("history") || [];
